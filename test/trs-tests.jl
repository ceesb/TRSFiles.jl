using Trsfile
import Trsfile: PRIMTYPES, HEADER_TAGS, TraceParam
...
function genheader(ntraces = 100, nsamples = 200, ntitle = 16)
    ...
    for (k,v) in HEADER_TAGS
        ...
        elseif datatype == :trace_set_parameters
            ...
            header[Trsfile.TRACE_SET_PARAMETERS] = tsp     
        ...
        elseif datatype == :trace_parameter_definitions
            ...
            header[Trsfile.TRACE_PARAMETER_DEFINITIONS] = tpd
        end
    end

    header[Trsfile.NUMBER_TRACES] = ntraces
    header[Trsfile.NUMBER_SAMPLES] = nsamples
    header[Trsfile.TITLE_SPACE] = ntitle
    header[Trsfile.SAMPLE_CODING] = rand(keys(Trsfile.PRIMTYPES))
    return header
end
...
function test()
    ...
    header[Trsfile.LENGTH_DATA]

    samples = rand(sampletype, nsamples, ntraces)
    title = rand(UInt8, ntitle, ntraces)
    data = rand(UInt8, header[Trsfile.LENGTH_DATA], ntraces)
    ...
    if !isnothing(keys)
        for key in keys
            ...
            tp = trsin.header[Trsfile.TRACE_PARAMETER_DEFINITIONS][key]
            offset = tp.offset
            len = tp.len
            width = Trsfile.PRIMTYPES[tp.typ].width
            eltyp = Trsfile.PRIMTYPES[tp.typ].eltype
            for t in 1 : ntraces
                @views begin
